...
def process_video(input_path, output_path):
    ...
    try:
        ...
        if frame_files and dominant_emotion != 'neutral':
            ...
            if pose_files:
                try:
                    ...
                    logger.info(f"Stored {len(pose_files)} pose files in environment")
                except Exception as e:
                    logger.error(f"Error serializing pose files: {e}")
        # ============= END POSE EXTRACTION =============
        # Annotate video
        ...
        # Save CSV
        csv_path      = f"{base_name}_analysis.csv"
        with open(csv_path, 'w') as f:
            f.write("video_path,transcription,emotion,duration_seconds,emotion_keywords_found,annotations_found,frames_extracted\n")
            f.write(f'"{video_name}","{transcription}","{dominant_emotion}",{duration:.2f},"{metadata["emotion_keywords_found"]}","{metadata["annotations_found"]}",{frames_extracted}\n')
        
        # Store frame files info for Lambda handler
        if frame_files:
            try:
                os.environ['FRAME_FILES'] = json.dumps(frame_files)
                logger.info(f"Stored {len(frame_files)} frame files in environment")
            except Exception as e:
                logger.error(f"Error serializing frame files: {e}")
